...
def apply_soa_layout(plan):
    ...
    via --soa so AoS remains the default for size-focused packs.
    """
    plan = dict(plan)
    plan["soa"] = True
    classes = {}
    for cname, cl in plan["classes"].items():
        cl = dict(cl)
        dims = 2 if cl["two_d"] else 3
        pos_names = ("pos_x", "pos_y", "pos_z")[:dims]
...
def emit_engine(plan, analyses, used_apis):
    ...
    for cname, cl in sorted(plan["classes"].items()):
        ...
        extra = ""
        if cl.get("soa_dims"):
            extra = ", pos SoA float[%d]" % cl["soa_dims"]
    ...
    for cname, cl in sorted(plan["classes"].items()):
        ...
        if cl.get("soa_dims"):
            ...
            for o in cl["instances"]:
                coords = [o["pos"][0], o["pos"][1]]
                if dims == 3:
                    coords.append(o["pos"][2])
                parts = ["%sf" % repr(float(v)) for v in coords]
...
def pack(root, outdir, soa=False):
...
def main():
    ...
    if len(args) != 1 or outdir is None:
        sys.stderr.write(
            "usage: unity_pack.py <project-dir> -o <out-dir> [--soa]\n")
        return 2
    try:
        plan = pack(args[0], outdir, soa=soa)
    except PackError as e:
        sys.stderr.write("unity_pack: %s\n" % e.message)
        return 1
    sys.stderr.write(
        "unity_pack: %d classes, 2d=%s, soa=%s, wrote %s/{engine.c,data.c,engine_draw.h}\n"
        % (len(plan["classes"]), plan["two_d"], plan.get("soa"), outdir))
    for name, cl in sorted(plan["classes"].items()):
        sys.stderr.write("  %s n=%d size=%d idx=%s%s\n"
                         % (name, cl["n"], cl["size"], cl["idx_ty"],
                            (" soa_dims=%d" % cl["soa_dims"])
                            if cl.get("soa_dims") else ""))
    return 0

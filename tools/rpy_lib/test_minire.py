...
if sys.implementation.name != 'minipy':
    ...
    # ---- rejection tests --------------------------------------------------
    # Every pattern outside the supported subset must raise MinreError. The
    # failure mode being guarded against is NOT "feature missing" but "feature
    # silently reinterpreted as a literal": before this check, `a|b` both
    # failed to match "b" and did match the literal "a|b". Wrong in both
    # directions, with no diagnostic.

    # Malformed patterns -- CPython's re rejects these too.
    _malformed = ['a)', '(ab', '[abc', '[]', '[z-a]', '*abc', '+a',
                  'a**', 'a+*', 'a?{2}', '(?P<n>a', '[a\\', 'a\\']
    # Well-formed but outside minire's subset -- CPython ACCEPTS these, so a
    # silent misparse here would never surface as an error anywhere.
    _unsupported = ['a|b', 'a{2}', 'a{2,3}', 'a{2,}', '(?:ab)c', '(?P<n>a)',
                    '(?=a)', '(?!a)', '(?<=a)b', '(?<!a)b', '(?i)a',
                    '\\bfoo', '\\Bfoo', '\\Afoo', '\\Zfoo', '(ab)+', '(ab)*',
                    '(ab)?', '(a)\\1', '\\xFF', '\\u0041']

    for _p in _malformed:
        _caught = 0
        try:
            compile(_p)
        except MinreError:
            _caught = 1
        if _caught == 0:
            raise AssertionError("minire accepted malformed pattern %r" % _p)
        # CPython must agree that it is malformed.
        _cpy_ok = 0
        try:
            _stdre.compile(_p)
            _cpy_ok = 1
            ...
            pass
        if _cpy_ok == 1:
            raise AssertionError("expected %r to be malformed for re too" % _p)

    for _p in _unsupported:
        _caught = 0
        try:
            compile(_p)
        except MinreError:
        ...
        if _caught == 0:
            raise AssertionError(
                "minire silently accepted unsupported pattern %r "
                "(it must raise, not reinterpret)" % _p)
        # These are valid regexes -- confirming that makes the point that
        # nothing else would have caught a silent misparse.
        _stdre.compile(_p)

...
extents. Scripts that call `transform.Rotate` or `transform.LookAt` keep a
live local quaternion (`_Class_rot_*`) and refresh that basis each call
(`Rotate` = Space.Self degrees; `LookAt` = LookRotation toward target,
default world up).

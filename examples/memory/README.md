...
* Instrumentation is uniform: every dereference is checked, including ones the
  static pass could prove safe. Expect roughly 30x on pointer-chasing loops.
  Using `--check-memory`'s whole-program result to elide provably-safe checks
  is the obvious next step, and is something Fil-C cannot do, having no
  whole-program view.
